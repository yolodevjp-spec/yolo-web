...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const articles = (articlesData as Record<string, { slug: string; title: string; excerpt: string }[]>).marketing || [];
...
export default async function ArticlePage({ params }: Props) {
  const { slug } = await params;
  const articles = (articlesData as Record<string, { slug: string; title: string; excerpt: string; date: string }[]>).marketing || [];
  const article = articles.find((a) => a.slug === slug);
  if (!article) notFound();
  return (
    <div className="min-h-screen bg-black text-white flex-1">
      <main className="max-w-3xl mx-auto px-4 py-10">
        <Link href="/marketing" className="text-gray-400 hover:text-white text-sm mb-6 inline-block">← Marketing一覧</Link>
        <article>
